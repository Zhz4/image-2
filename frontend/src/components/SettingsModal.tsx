...
export default function SettingsModal() {
  ...
  const createProfileImportUrl = (
    ...
    if (profile.provider === "openai") {
      ...
      if (includeApiKey && profile.apiKey.trim())
  ...
  const copyProfileImportUrl = async (
    ...
    try {
      ...
      showToast(
        includeApiKey ? "导入 URL 已复制（包含 API Key）" : "导入 URL 已复制",
        "success",
      );
    } catch (err) {
                  ...
                  {apiProxyAvailable && activeProfile.provider === "openai" && (
                    <div className="block">
                      <div className="mb-1.5 flex items-center justify-between">
                        ...
                        >
                          <span
                            className={`inline-block h-3 w-3 transform rounded-full bg-white shadow transition-transform ${activeProfile.apiProxy ? "translate-x-[14px]" : "translate-x-[2px]"}`}
                          />
                        </button>
                      </div>
                      <div
              ...
              {activeTab === "data" && (
                <div className="space-y-4">
                  <div className="rounded-2xl bg-blue-50/50 p-4 border border-blue-100/50 dark:bg-blue-500/5 dark:border-blue-500/10 flex items-start gap-3">
                    <svg
                      className="w-5 h-5 text-blue-500/70 dark:text-blue-400/70 shrink-0 mt-0.5"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                      ...
                      <label className="flex items-center gap-2 cursor-pointer group">
                        <div className="relative flex items-center justify-center">
                          <input
                            type="checkbox"
                            checked={clearConfig}
                            onChange={(e) => setClearConfig(e.target.checked)}
        ...
        createPortal(
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <div
              ...
              className="relative z-10 w-full max-w-lg rounded-3xl border border-white/50 bg-white/95 p-5 shadow-2xl ring-1 ring-black/5 animate-modal-in dark:border-white/[0.08] dark:bg-gray-900/95 dark:ring-white/10 overflow-y-auto overscroll-contain max-h-[85vh] custom-scrollbar"
            >
              ...
              <div className="mb-6 rounded-2xl bg-blue-50/50 p-4 border border-blue-100/50 dark:bg-blue-500/5 dark:border-blue-500/10">
                ...
                <div
                  data-selectable-text
                  className="mb-3 text-[11px] leading-relaxed text-blue-600/80 dark:text-blue-400/80"
                >
                  复制提示词发给 LLM，可根据 API
              ...
              {customProviderImportError && (
                <div
                  data-selectable-text
                  className="mt-2 rounded-lg bg-red-50 px-3 py-2 text-xs text-red-500 dark:bg-red-500/10 dark:text-red-300"
                >
                  {customProviderImportError}
                </div>
              )}
