...
import { imageBlobToPngBlob } from '../lib/canvasImage'
...
export default function ImageContextMenu() {
  ...
  const handleCopy = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      await copyBlobToClipboard(await imageBlobToPngBlob(blob))
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    try {
      const src = await getOriginalImageSrc()
      const blob = await imageSourceToBlob(src)
      downloadBlob(blob, `image-${Date.now()}.${getImageBlobExtension(blob)}`)
