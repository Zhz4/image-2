...
import { downloadBlob, getImageBlobExtension, imageSourceToBlob } from './lib/imageSource'
...
async function imageSourceToBytes(src: string): Promise<{ ext: string; bytes: Uint8Array }> {
  if (src.startsWith('data:')) return dataUrlToBytes(src)

  const blob = await imageSourceToBlob(src)
  return {
    ext: getImageBlobExtension(blob),
...
export async function exportData(options: ExportOptions = { exportConfig: true, exportTasks: true }) {
  try {
    ...
    downloadBlob(blob, `gpt-image-playground-${formatExportFileTime(new Date(exportedAt))}.zip`)
