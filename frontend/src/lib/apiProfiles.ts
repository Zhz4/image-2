...
const DEFAULT_CODEX_CLI = import.meta.env.VITE_DEFAULT_CODEX_CLI === 'true'
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  const storedBaseUrl = typeof record.baseUrl === 'string' ? record.baseUrl : defaults.baseUrl
  // Migrate legacy OpenAI base URLs to the current default when the default has been overridden via env
  const LEGACY_OPENAI_BASE_URLS = ['https://api.openai.com/v1', 'https://api.openai.com']
  const resolvedBaseUrl =
    provider === 'openai' &&
    DEFAULT_BASE_URL !== 'https://api.openai.com/v1' &&
    LEGACY_OPENAI_BASE_URLS.includes(storedBaseUrl)
      ? DEFAULT_BASE_URL
      : storedBaseUrl
